 ...
 *
 * @details The occupation block is stored conj-first, D_ab = conj(c_a) c_b:
 *   D = |a|^2    conj(a)b |   = | (1+Mz)/2    (Mx+iMy)/2 |
 ...
 *   My = Im(occ[1] - occ[2])                            (conj-first storage)
 *   Mz = Tr(rho * sigma_z) = occ[0] - occ[3]            (real part)
 * where occ = {|a|^2, conj(a)b, conj(b)a, |b|^2} from becp coefficients.
 *
