    ...
    for (std::size_t ifunc = 0; ifunc < functional_ids.size(); ++ifunc)
    {
        ...
        for (std::size_t ieps = 0; ieps < 4; ++ieps)
        {
            ...
            const double finite_difference
                = (evaluate(density + step, 0.0, nullptr)
                   - evaluate(density - step, 0.0, nullptr))
                  / (2.0 * step);
        ...
        const double inactive_finite_difference
...
TEST(LibxcSanitizer, GgaSigmaReverseMatchesTheWeightedEnergy)
{
    ...
    const std::array<std::array<double, 3>, 5> sigma_states
        = {{{{0.040, 0.010, 0.030}},
                              ...
                              XC_Functional_Libxc::LibxcWeightedDerivatives* const weighted)
    {
        ...
        {
            *weighted = XC_Functional_Libxc::make_libxc_weighted_derivatives(
                func, 2, 1, mask, rho, sigma, exc, vrho, vsigma);
        }
...
TEST(LibxcSanitizer, UnpolarizedSelfSigmaReverseMatchesTheWeightedEnergy)
{
    ...
    const auto evaluate = [&func, &mask, &density](
                              ...
                              XC_Functional_Libxc::LibxcWeightedDerivatives* const weighted)
    {
        ...
        {
            *weighted = XC_Functional_Libxc::make_libxc_weighted_derivatives(
                func, 1, 1, mask, density, sigma, exc, vrho, vsigma);
        }
    ...
    const double below_floor_finite_difference
    ...
    const double above_floor_finite_difference
